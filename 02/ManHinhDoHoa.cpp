...
std::vector<TamGiac*> ManHinhDoHoa::danhSachTamGiac;

void ManHinhDoHoa::drawAxes() {
    glColor3f(1.0f, 1.0f, 1.0f);   // Màu của trục Ox và Oy (trắng)

    glBegin(GL_LINES);
    glVertex2f(-10.0f, 0.0f); glVertex2f(10.0f, 0.0f);   // Trục Ox
    glVertex2f(0.0f, -10.0f); glVertex2f(0.0f, 10.0f);   // Trục Oy

    float tickSize = 0.2f;  // Độ dài vạch chia đơn vị
    for (int i = -10; i <= 10; ++i) {  // Vẽ từng vạch chia đơn vị
        if (i == 0) continue;
...
void ManHinhDoHoa::renderScreen() {
    ...
    drawAxes();
    for (int i = 0; i < danhSachTamGiac.size(); i++) {
        danhSachTamGiac[i]->VeTamGiac();
    }
...
void ManHinhDoHoa::ThemTamGiac(TamGiac* tg) {
    danhSachTamGiac.push_back(tg);
}
...
void ManHinhDoHoa::HienThi(int argc, char** argv) {
    ...
    glutInitWindowSize(600, 600);   // Kích thước cửa sổ đồ họa
    glutCreateWindow("Ve He Truc Toa Do va Tam Giac");
